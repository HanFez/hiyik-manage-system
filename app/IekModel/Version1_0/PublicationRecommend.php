...
class PublicationRecommend extends IekModel
{
    const PID_NAME = 'publication_id';
    const UID_NAME = 'person_id';

    protected $table = 'tblPublicationRecommends';

    public static function getRecommendCount($pid, $begin = null, $end = null) {
        $count = 0;
        if(!is_null($pid)) {
            $condition = [
                [self::PID_NAME, $pid],
    ...
    public static function getRecommends($pid = null) {
        $persons = [];
        if(is_null($pid)) {
            return $persons;
        }
