...
use App\IekModel\Version1_0\Product\PosterFrameMaterialDefine;
use Illuminate\Support\Facades\DB;

class FrameMaterialDefineController extends Controller
{
/**
     * FrameMaterialDefine's add page
     */
    public function add(){
        ...
        return view('product.define.frameMaterialDefine',compact('materials','mcs','facades'));
    }
    /**
     * FrameMaterialDefine's add data deal
     */
    public function create(){
        ...
        try{
            DB::beginTransaction();
            $fmd = new PosterFrameMaterialDefine();
            $fmd->name = $req['name'];
    ...
    /**
     * FrameMaterialDefine's all data list
     */
    public function listFMD(){
        $model = new PosterFrameMaterialDefine();
        $type = 'frame-material-define';
        $getList = new IndexController();
        $result = $getList->tableList($model,$type);
        return $result;
    }
    /**
     * FrameMaterialDefine's data detail
     */
    public function showFMD(){
        //
    }
    /**
     * FrameMaterialDefine's edit page
     */
    public function edit($id){
        ...
        $facades = Facade::where(IekModel::CONDITION)->get();
        $action = 'edit';
        $fmd = PosterFrameMaterialDefine::find($id);
        $rgba = turnRgba($fmd->rgba);
        return view('product.define.frameMaterialDefine',compact('materials','mcs','facades','action','fmd','rgba'));
    }
    /**
     * FrameMaterialDefine's edit data deal
     */
    public function update($id){
        ...
        try{
            DB::beginTransaction();
            $fmd = PosterFrameMaterialDefine::where(IekModel::ID,$id)
                ->update([
        ...
        return $this->curd(Errors::OK,Errors::FAILED,"修改成功","修改失败",$fmd);
    }
    /**
     * delete FrameMaterialDefine's record
     */
    public function del(){
        $model = new PosterFrameMaterialDefine();
    ...
    /**
     * recover FrameMaterialDefine's record
     */
    ...
    public function verify($req,$id){
        ...
        $frame = PosterFrameMaterialDefine::where(IekModel::NAME,$name)
            ->where(IekModel::MATERIAL_ID,$mid)
            ->where(IekModel::CATEGORY_ID,$cid)
            ->first();
        if(is_null($id)){
            if(!is_null($frame)){
                return $this->viewReturn(Errors::EXIST,"该记录已存在",$name);
            }
        }else{
            if(!is_null($frame) && $frame->id != $id){
                return $this->viewReturn(Errors::EXIST,"该记录已存在",$name);
            }
        }
    }
