...
$ftd = isset($ftd)?$ftd:null;
$rgba = isset($rgba)?$rgba:null;
$path = \App\IekModel\Version1_0\Constants\Path::FILE_PATH;
?>
<div class="widget-box">
    <div class="widget-title"> <span class="icon"> <i class="icon-align-justify"></i> </span>
        <h5>@if($action == 'edit'){{"修改玻璃材料定义"}}@else{{"定义玻璃材料"}}@endif</h5>
    </div>
    <div class="widget-content nopadding">
        <form class="form-horizontal" id="form-front-material-define">
            <div class="control-group">
                <label class="control-label">名称：</label>
                <div class="controls">
                    <input type="text" name="name" value="{{$action == 'edit'?$ftd->name:''}}">
                </div>
            </div>
            <div class="control-group">
                <label class="control-label">描述：</label>
                <div class="controls">
                    <input type="text" name="des" value="{{$action == 'edit'?$ftd->description:''}}">
                </div>
            </div>
            <div class="control-group">
                    ...
                    <select name="material" id="material-id" style="width: 220px;">
                        @if($action == 'edit')
                            @foreach($materials as $material)
                                <option value="{{$material->id}}" {{$material->id == $ftd->material_id ? 'selected="selected"':''}}>
                                    {{$material->name}}
                                </option>
                            @endforeach
                        @else
            ...
            <div class="control-group">
                ...
                <div class="controls">
                    <select name="category" id="category-id" style="width: 220px;">
                        @if($action == 'edit')
                            @foreach($mcs as $mc)
                                <option value="{{$mc->id}}" {{$mc->id == $ftd->category_id ? 'selected="selected"':''}}>
                                    {{$mc->name}}
                                </option>
                            @endforeach
                        @else
                ...
                <div class="controls">
                    @if($action == 'edit')
                        @foreach($facades as $facade)
                            <div class="thumbnail {{$facade->id == $ftd->facade_id ? 'active' : ''}}">
            ...
            <div class="control-group">
                ...
                <div class="controls">
                    <input type="number" name="phyWidthMax" value="{{$action == 'edit'?$ftd->phy_width_max:''}}">
                </div>
            </div>
            <div class="control-group">
                <label class="control-label">最小宽度(mm)：</label>
                <div class="controls">
                    <input type="number" name="phyWidthMin" value="{{$action == 'edit'?$ftd->phy_width_min:''}}">
                </div>
            </div>
            <div class="control-group">
                <label class="control-label">最大高度(mm)：</label>
                <div class="controls">
                    <input type="number" name="phyHeightMax" value="{{$action == 'edit'?$ftd->phy_height_max:''}}">
                </div>
            </div>
            <div class="control-group">
                <label class="control-label">最小高度(mm)：</label>
                <div class="controls">
                    <input type="number" name="phyHeightMin" value="{{$action == 'edit'?$ftd->phy_height_min:''}}">
                </div>
            </div>
            <div class="control-group">
                <label class="control-label">物理厚度(mm)：</label>
                <div class="controls">
                    <input type="number" name="phyDepth" value="{{$action == 'edit'?$ftd->phy_depth:''}}">
                </div>
            </div>
            <div class="control-group">
                <label class="control-label">颜色名称：</label>
                <div class="controls">
                    <input type="text" name="colorName" value="{{$action == 'edit'?$ftd->color_name:''}}">
                </div>
            </div>
            <div class="control-group">
            ...
            <div class="control-group">
                ...
                <div class="controls">
                    <input type="text" name="price" value="{{$action == 'edit'?$ftd->price:''}}">
                </div>
            </div>
            <div class="control-group">
                <label class="control-label">单位：</label>
                <div class="controls">
                    <input type="text" name="priceUnit" value="{{$action == 'edit'?$ftd->price_unit:''}}">
                </div>
            </div>
            <div class="control-group">
                <label class="control-label">货币：</label>
                <div class="controls">
                    <input type="text" name="currency" value="{{$action == 'edit'?$ftd->currency:''}}">
                </div>
            </div>
            <div class="form-actions">
                <a type="submit" class="btn btn-success" id="save-front-material-define">保存</a>
            </div>
        </form>
    </div>
</div>
<script>
    var form = $('#form-front-material-define');
    $('#facade .thumbnail').on('click', function () {//选中当前图片
        $('#facade .thumbnail').removeClass('active');
        $(this).addClass('active');
    });
    var action = '{{$action}}';
    var id = '{{is_null($ftd) ? NULL : $ftd->id}}';
    $('#save-front-material-define').unbind('click').bind('click',function(){
        ...
        if(action == 'edit'){
            ajaxData('put', 'new_pro/updateFrontMaterialDefine/'+id, function (result) {
                if(result) {
                    form.append(result);
                    //form.val('');
                }
            ...
            ajaxData('post', 'new_pro/createFrontMaterialDefine', function (result) {
                if(result) {
                    form.append(result);
                    //form.val('');
                }
